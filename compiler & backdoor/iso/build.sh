...
make_basefs() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "haveged intel-ucode mkinitcpio-nfs-utils nbd" install
}
...
make_customize_airootfs() {
    ...
    cp ${script_path}/airootfs/root/.bashrc ${work_dir}/${arch}/airootfs/root/.bashrc
